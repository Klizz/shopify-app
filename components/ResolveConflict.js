...
const ResolveConflict = ({
  ...
  useEffect(() => {
    ...
    if (v.length <= 1) p.hasPrevious = true;
  ...
  const handleSelectedPrice = price_selected => {
    setHasPaginator({ ...hasPaginator, price_selected });
    move(0, price_selected);
  };
  const move = (step, price_selected) => {
    if (price_selected === undefined)
      price_selected = hasPaginator.price_selected;
    const v = variants_with_conflict;
  ...
  const footer = (
    <Stack>
      <Stack.Item fill>{pagination}</Stack.Item>
      <Stack.Item>
  ...
  return (
    ...
    >
      <Modal.Section>
        <TextContainer>
          <div>
            Nuestros expertos sugieren diferentes impuestos que los que usted ha
            indicado Seleccione cual es el correcto
          </div>
          <Stack distribution="fillEvenly">
            ...
            <Stack.Item>
              ...
              <div>Impuestos: seleccione a continuacion</div>
            </Stack.Item>
          </Stack>
          <Stack distribution="fillEvenly">
            <Card sectioned>
              ...
              <p className="text-center">
                Impuesto ${calculated_duty_original}
              </p>
              <div className="text-center mt-10">
                <Button
                  onClick={() => handleSelectedPrice("original")}
                  primary={
                    hasPaginator.price_selected === "original" &&
            ...
            <Card sectioned>
              ...
              <div className="text-center mt-10">
                <Button
                  onClick={() => handleSelectedPrice("recommended")}
                  primary={hasPaginator.price_selected === "recommended"}
                  icon={
                    hasPaginator.price_selected === "recommended" &&
                    MobileAcceptMajorMonotone
                  }
                >
                  Seleccionar
