  ...
  switch (action.type) {
    case types.SET_VARIANTS: {
      ...
      return {
        ...
        variants_with_conflict: variants
          .filter(element => element.status === "Conflicto")
          .map(element => {
            return { ...element, price_selected: null };
          })
      };
    }
    ...
    case types.SOLVE_VARIANT: {
      const { id } = action;
      const index = state.variants_with_conflict.findIndex(
        element => element.id === id
      );
      //const variant_in_modal = { ...state.variants_with_conflict[index] }
      const variant_in_modal = JSON.parse(
        JSON.stringify(state.variants_with_conflict[index])
      );
    ...
    case types.STEP_VARIANT: {
      ...
      const currentIndex = state.variants_with_conflict.findIndex(
        element => element.id === state.variant_in_modal.id
      );
      const variant = { ...state.variant_in_modal, price_selected: selected };

      let newVariants = JSON.parse(
        JSON.stringify(state.variants_with_conflict)
      );
      newVariants[currentIndex] = variant;

      const nextVariant = { ...newVariants[next_index] };
      const remaining = newVariants.filter(
        element =>
